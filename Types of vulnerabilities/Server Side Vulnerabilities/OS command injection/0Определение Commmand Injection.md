...
[[#Пути экcплуатации OS command injection]]
[[#Предотвращение атак OS command injection]]
...
> [!hint] Разделители комманд
> Позволяют соединять несколько команд в одной строке. Работают и в Windows и в Unix подобных системах
```
&
&&
|
...
```
**Смысл:**
- `cmd1 & cmd2` - запустить вторую команду независимо от результата первой
- `cmd1 && cmd2` - вторая выполнится только если первая успешна
- `cmd1 || cmd2` - вторая выполнится только если первая завершилась с ошибкой
- `cmd1 | cmd2` - вывод первой передается на вход второй 

> [!warning] Только Unix-подобные системы
```
;
\n - перевод строки
0x0a - перевод строки
```
`cmd1; cmd2` - безусловное последовательное выполнение команд

Так же в Unix-подобных системах можно использовать обратные кавычки или символ `$` для встроенного выполнения внедренной команды внутри исходной
команда исполняется внутри другой команды, а ее вывод подставляется как текст
```
`command`
$(command)
```
тут происходит вычисление значения и его последующее подстановка в команду, например
```
echo $(id) # функция id возвращает 123
результат: 123
```
Разные метасимволы shell имеют слегка различное поведение и это влияет на их работоспособность в определенных ситуациях. Это может определять, позволят ли они получить вывод команды в текущем канале(in band) или пригодны только для слепой эксплуатации
Так же бывают ситуации когда ввод оказывается внутри кавычек в исходной команде. В таком случае нужно завершить контекст кавычек с помощью `'` или `"` а затем использовать подходящие символы shell для инъекции команды
## Предотвращение атак OS command injection
Самый эффективный способ предотвращения никогда не вызывать OS command из кода уровня приложения. Почти всегда существуют другие способы реализовать необходимую функциональность, используя безопасные платформенные API.
Если все же приходится вызывать OS command с пользовательским вводом, необходимо выполнять строгую проверку ввода
- Проверка по разрешающему списку допустимых значений
- Проверка, что ввод - число
- Проверка, что ввод содержит не только буквенно-цифровые символы без иной синтаксической информации и пробелов