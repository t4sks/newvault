...
---
### Получение нескольких значений в одном столбце
иногда запросы могут возвращать только один столбец, но можно получить несколько несколько значений в одном столбце пример для [[#Oracle]]
```sql
' UNION SELECT username || '~' || password FROM users--
```
Здесь `||` это оператор объединения строк а `'~'` разделитель в результате строка будет `username~password`
