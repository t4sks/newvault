...
когда сервер выдает пользователю доступ к функциям которые он не должен видеть или запускать, потому что проверка "может ли пользователь выполнить эту функцию" отсутствует или неверна 