...
давайте сфокусируемся на втором метода и увидим как мы или злоумышленники могут управлять пользователями в Windows. Первое свойство это использовать графическую утилиту "Computer Managment" или `lusrmgr.msc`. Второе это использовать комманды которые представлены ниже 
...
Далее созданый юзер не может дать атакующему много как обычный пользователь с обычными правами. Для преодоления этого злоумышленники добавляют аккаунт в привилегированные, это отслеживается событием 4732. Наиболее частые группы Administrators и Remote Desktop Users
